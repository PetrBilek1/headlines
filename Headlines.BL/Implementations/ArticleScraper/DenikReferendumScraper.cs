    ...
    public sealed class DenikReferendumScraper : ArticleScraperBase
    {
        ...
        protected override string GetTitle(HtmlDocument document)
            => document.DocumentNode
                .SelectSingleNode("//div[contains(concat(' ', @class, ' '), ' header ')]//h2")
                ?.InnerText
                .Trim()
            ?? string.Empty;
        ...
        protected override string GetAuthor(HtmlDocument document)
            => string.Join(
                ", ",
                document.DocumentNode
                    .SelectNodes(".//*[contains(concat(' ', @class, ' '), ' articleDetailAuthorName ')]")
        ...
        protected override string GetPerex(HtmlDocument document)
            => document.DocumentNode
                .SelectSingleNode("//*[contains(concat(' ', @class, ' '), ' perex ')]")
                ?.InnerText
                .Trim()
            ?? string.Empty;
        ...
        protected override List<string> GetParagraphs(HtmlDocument document)
            => document.DocumentNode
                .SelectNodes("//div[contains(concat(' ', @class, ' '), ' text ')]/*[self::p or self::h3]")
                ?.Where(x => !string.IsNullOrWhiteSpace(x.InnerText))
                .Select(x => x.InnerText.Trim())
                .ToList()
            ?? new List<string>();
