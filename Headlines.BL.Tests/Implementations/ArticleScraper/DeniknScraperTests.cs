...
{
    public sealed class DeniknScraperTests : ScraperTestBase<DeniknScraper>
    {
        [Theory]
        [InlineData("001")]
        [InlineData("002")]
        ...
        {
            //Arrange
            var html = await ScraperDataLoader.GetHtmlAsync("DenikN", index);
            var expected = await ScraperDataLoader.GetExpectedAsync("DenikN", index);
