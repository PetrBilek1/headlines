...
{
    public sealed class BleskScraperTests : ScraperTestBase<BleskScraper>
    {
        [Theory]
        [InlineData("001")]
        [InlineData("002")]
        ...
        {
            //Arrange
            var html = await ScraperDataLoader.GetHtmlAsync("Blesk", index);
            var expected = await ScraperDataLoader.GetExpectedAsync("Blesk", index);
